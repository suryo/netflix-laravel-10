    ...
    <div class="bg-admin-card rounded-xl border border-admin-border p-6">
        <form action="{{ route('admin.categories.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Action, Comedy..."
            ...
            <div class="mb-6 p-4 bg-white/5 rounded-lg border border-admin-border hover:border-red-500/30 transition-colors">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_adult" value="1" {{ old('is_adult') ? 'checked' : '' }}
            ...
            <div class="flex items-center gap-3">
                <button type="submit" class="px-6 py-3 bg-admin-accent hover:bg-blue-600 text-white font-medium rounded-lg transition-colors">Save Category</button>
